...
D <- vegdist(d[, c(9:25)], method = "euclidean")
m3 <- cmdscale(D, k = 1)
...
tSNE follows a very similar idea, utilising the joint probability distribution (rather than the conditional) which is faster to compute, as it has a simpler gradient. The `Rtsne` package used in this workflow utilises a variant of the Barnes-Hut algorithm to approximate the gradient [@van2014accelerating].  
...
In this workflow several methods for exploring and understanding multivariate patterns in species composition data, graphically, are presented. There are some unique challenges in dealing with species composition data. Some of the problems of multivariate data analysis are well known and discussed extensively in [@tabachnick2007using], however the less obvious major problem in species composition data is that it *is compositional*. Because the sum of components is always constrained to a constant (*e.g* 1 in the case of proportions), spurious correlations may result and statistical methods designed for unconstrained sample spaces may not be appropriate (for a discussion in the context of microbiome data see [@gloor2016compositional]). This workflow makes little attempt to address these issues because it is designed to be exploratory, not confirmatory. Nevertheless, it does permit (within the constraints of domain-specific knowledge) hypothesis generation and provides some tools to aid in the exploration of the species abundance behind the embeddings.  
